...
class HomographyTransformer:
    ...
    def homographize_image(self, image):
        """
        Takes in an image and returns a new image with the same size and
        same pixel values, but with the pixels transformed to the ground plane.
        """
        im_dst = cv2.warpPerspective(image, self.h, (image.shape[1], image.shape[0]))
        return im_dst
