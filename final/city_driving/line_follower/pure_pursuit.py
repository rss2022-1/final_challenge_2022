...
class PurePursuit(object):
    ...
    def find_lookahead_point(self, lane_segments):
        ...
        with radius lookahead distance intersects with line segments. Returns the average of intersected points.
        """
        ...
        if len(lane_segments) == 0:
            ...
            return (0,0,0)
        lookahead_line = self.line((0, self.px_lookahead), (1000, self.px_lookahead)) #straight line px_lookahead ahead
        intersections = []
        for i in range(len(lane_segments)):
            l = self.line(lane_segments[i][1:3], lane_segments[i][3:5]) #(x1,y1) to (x2,y2)
...
def test_get_lanes():
    masks_path = os.path.abspath(os.getcwd()) + "/../test_imgs/line_masks/"
    for i in range(0, 6):
