...
class LaneSegmenter():
    """
    A class for segmenting an image based on color to find lanes.
    Subscribes to: /zed/zed_node/rgb/image_rect_color (Image): the live RGB image from the onboard camera.
    Publishced to: /lane_segmenter/lane_mask (Image): a binary image showing the lane mask.
    """

    def __init__(self):
        self.image_sub = rospy.Subscriber("/zed/zed_node/rgb/image_rect_color", Image, self.image_callback)
        self.mask_pub = rospy.Publisher("/lane_segmenter/lane_mask", Image, queue_size=1)
